<template>
  <div class="formulario">

    <h1> Editar recorrido </h1>
        <form @submit.prevent="guardar">

            <v-text-field
                v-model="nuevoObjeto.nombre"
                label="Nombre"
                type="text"
            ></v-text-field>
          <v-text-field
              v-model="nuevoObjeto.distancia"
              label="Distancia en km"
              type="number"
              min="1"
          ></v-text-field>
          <v-text-field
              v-model="nuevoObjeto.duracion"
              label="Duración en minutos"
              type="number"
              min="1"
          ></v-text-field>

            <v-select
                v-model="nuevoObjeto.ambito"
                :items="select"
                label="Ámbito"
                data-vv-name="nuevoObjeto.ambito"
                required

            ></v-select>
          <v-text-field
              v-model="nuevoObjeto.dia"
              label="Día"
              type="date"
          ></v-text-field>

          <v-btn
              class="ml-auto mt-4"
              type="submit"
              color="primary"
          >
            Actualizar
          </v-btn>
        </form>

    <div v-if="envio === true">
      <div v-if="cantErrores" class="errores">
        <ul>
          <li v-for="error in errores" :key="error"> {{error}} </li>
        </ul>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "Editar",

  data: function () {
    return {
      nuevoObjeto:{
        nombre: "",
        duracion :  "",
        distancia :  "",
        ambito:  "",
        dia: "",
      },
      select: [{ text: 'Selecione uno', value: null }, 'Ciudad', 'Montaña'],
      errores: [],
      envio: false,
      array: [],
      localStorage: [],
    }
  },

  mounted: function(){
    
    console.log('ID: ' + this.$route.params.id);
    this.localStorage = JSON.parse(localStorage.getItem("data"));
    console.log(this.localStorage);

    for(var i=0; i < this.localStorage.length; i++) {
      if(this.localStorage[i].id == this.$route.params.id) {
        this.nuevoObjeto = this.localStorage.splice(i, 1)[0];
        console.log(this.nuevoObjeto);
      }
    }
  },
  computed: {
    cantErrores: function () {
      return this.errores.length;
    }
  },

  methods: {
    guardar: function () {
      this.envio = true;
      this.errores = [];
      if (!this.nuevoObjeto.nombre) {
        this.errores.push('El nombre del recorrido es obligatorio')
      }
      if (!this.nuevoObjeto.distancia) {
        this.errores.push('La distancia es obligatoria')
      }
      if (!this.nuevoObjeto.distancia > 1) {
        this.errores.push('La distancia debe ser mayor a 0')
      }
      if (!this.nuevoObjeto.duracion) {
        this.errores.push('La duración es obligatoria')
      }
      if (!this.nuevoObjeto.duracion > 1) {
        this.errores.push('La duración debe ser mayor a 0')
      }
      if (this.nuevoObjeto.ambito == null) {
        this.errores.push('Debe ingresar un ámbito')
      }
      if (this.nuevoObjeto.dia == null) {
        this.errores.push('Debe ingresar una fecha válida')
      }

      if (this.errores.length == 0) {
        var formData = new FormData();

        for (let item in this.nuevoObjeto){
        formData.append(item, this.nuevoObjeto[item]);
        }

        fetch('https://apivelobike.000webhostapp.com/api/guardar.php', {
          method: 'POST',
          body: formData,
        })
        .then(res=> res.json())
        .then(res=> {
          console.log(res);
          this.array = res;

           localStorage.setItem("data", JSON.stringify(this.array));
        });
      
        this.$router.push('/recorrido');
      }
    },


    },


}


</script>

<style>
.formulario {
  width: 80%;
  margin: 2em auto;
}

.errores {
  color: white;
  width: 100%;
  margin: 1em auto;
  text-align: center;
}

.errores li {
  color: white;
  font-weight: bold;
  border: none;
  padding: 10px;
  margin: .5em auto;
  border-radius: 5px;
  background: #8d1414;
}

.errores li:hover {
  background-color: #e2221f8e;

}


.envioOk p {
  margin: .5em auto;
}

</style>